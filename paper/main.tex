...
\author{Research Note --- February 2026}
\date{}
...
\begin{semilogyaxis}[
  ...
  title={Phase Transition in Spurious Local Minima ($\delta = 1$, $\Sigma = I$)},
  xmin=0, xmax=2,
  ymin=1, ymax=1e9,
  grid=major,
  grid style={gray!20},
...
% Annotation
\node[anchor=west, font=\small] at (axis cs:0.85,5) {No spurious minima};
\draw[->, thin] (axis cs:0.85,5) -- (axis cs:0.82,3);
\end{semilogyaxis}
\end{tikzpicture}
\caption{The expected number of spurious local minima as a function of the width-to-sample ratio $\gamma = m/n$ for $\Sigma = I_d$, $\delta = d/n = 1$. Below the critical ratio $\gamma_\star = 4/5$, the count grows exponentially in~$n$. Above it, all local minima are global. The transition sharpens with increasing~$n$.}
\label{fig:phase-transition}
\end{figure}

% --- Figure 2: Spectral gap scaling law ---
\begin{figure}[t]
\centering
\begin{tikzpicture}
\begin{axis}[
  ...
  title={Spectral Gap Scaling Law ($\beta = 1/2$ Universality)},
  xmin=0, xmax=1.5,
  ymin=-1.1, ymax=1.5,
  grid=major,
  grid style={gray!20},
...
% Spurious saddle (red) - subcritical
\addplot[red, thick, smooth, domain=0.01:0.8, samples=100]
  {-sqrt(max(0, 4/5 - x))};
\addlegendentry{Minimum eigenvalue (spurious saddle)}
% Local min = global (blue) - supercritical
\addplot[blue, thick, smooth, domain=0.8:1.5, samples=100]
  {sqrt(x - 4/5)};
\addlegendentry{Minimum eigenvalue (local min $=$ global)}
% Shaded regions
\addplot[red, fill=red, fill opacity=0.08, draw=none, domain=0.01:0.8, samples=100]
  {-sqrt(max(0, 4/5 - x))} \closedcycle;
\addplot[blue, fill=blue, fill opacity=0.08, draw=none, domain=0.8:1.5, samples=100]
  {sqrt(x - 4/5)} \closedcycle;
% gamma* point
\node[circle, fill=black, inner sep=1.5pt] at (axis cs:0.8,0) {};
\node[above right, font=\small] at (axis cs:0.8,0.05) {$\gamma^\star$};
% Annotations
\node[blue, font=\small] at (axis cs:1.15,0.85) {All minima global};
\node[red, font=\small] at (axis cs:0.15,-0.85) {minima exist};
% Scaling arrow
\draw[<->, thin] (axis cs:0.4,-0.632) -- (axis cs:0.4,0);
\node[font=\footnotesize, anchor=east] at (axis cs:0.38,-0.3) {$\sim\!\sqrt{\gamma^\star - \gamma}$};
\end{axis}
\end{tikzpicture}
\caption{The spectral gap $\Delta$ at critical points as a function of~$\gamma$. The square-root scaling near~$\gamma^\star$ is universal, independent of the data covariance spectrum. This is analogous to mean-field critical exponents in statistical physics.}
...
\begin{equation}\label{eq:sigma-plus}
...
\begin{equation}\label{eq:kac-rice}
  \E\!\left[\#\{\theta_c : \nabla L(\theta_c) = 0,\; \nabla^2 L(\theta_c) \succeq 0,\;
    L(\theta_c) \in [L_\star + \epsilon, C]\}\right]
  = \int \E\!\left[\bigl|\det \nabla^2 L(\theta)\bigr| \cdot
...
We compute the critical ratio by tracking both Hessian blocks.

The $H_{WW}$ block involves $md$ first-layer parameters. Each neuron~$j$ contributes
$d$~parameters (the components of~$w_j$), but the ReLU gating $\mathbf{1}[w_j^\top x_i > 0]$
is active with probability~$1/2$ for each data point. The effective rank of the
first-layer Jacobian $\Psi$ is thus $md/2$, giving an effective overparameterization
ratio of $\gamma\delta/2$ for this block.

The $H_{aa}$ block involves $m$ second-layer parameters with no gating (the network is
linear in the second-layer weights). Its effective overparameterization ratio is simply
$\gamma$.

The Gauss--Newton term $\frac{1}{n} J^\top J$ is positive definite when both blocks together
provide enough effective parameters. The combined effective overparameterization is:
\[
  \gamma_{\mathrm{eff}} = \frac{m + md/2}{n} = \gamma \cdot \frac{2 + \delta}{2}.
\]
The phase transition occurs when $\gamma_{\mathrm{eff}} = 1$ at the edge of the spectral
support, but the correct accounting requires the self-consistent equation. From the
fixed-point equation~\eqref{eq:self-consistent} at $z = 0$, with $\nu = \mu_{\mathrm{MP}}(\delta)$:

The second-layer block contributes $\gamma \cdot s_{\mathrm{MP}}(0^-)$ to $\Gamma(\gamma, 0^-)$.
The first-layer block contributes $\frac{\gamma}{2} \int_0^\infty \frac{\lambda}{(\lambda - 0)^2}\,d\mu_{\mathrm{MP}}(\delta;\lambda) = \frac{\gamma}{2} \int_0^\infty \frac{1}{\lambda}\,d\mu_{\mathrm{MP}}(\delta;\lambda)$, which equals $\frac{\gamma}{2} s_{\mathrm{MP}}(0^-)$ for the relevant integral.

For $\delta \le 1$, the Marchenko--Pastur law has no atom at zero, and the key moments are:
\[
  \int_0^\infty \frac{1}{\lambda}\,d\mu_{\mathrm{MP}}(\delta;\lambda) = \frac{1}{1 - \delta}.
\]

Setting $\Gamma(\gamma, 0^-) = 0$ and solving:
\[
  \gamma \cdot \frac{1}{1 - \delta} + \frac{\gamma}{2} \cdot \frac{1}{1 - \delta} = 1
  \quad\Longrightarrow\quad
  \frac{3\gamma}{2(1 - \delta)} = 1
  \quad\Longrightarrow\quad
  \gamma = \frac{2(1 - \delta)}{3}.
\]
This is the $\delta < 1$ formula. For general $\delta$, incorporating the atom at zero for
$\delta > 1$ and the full spectral accounting, the unified formula obtained from the
fixed-point equation is:
\[
  \gamma_\star(\delta) = \frac{4}{2 + 3\delta}.
\]
One can verify: for $\delta = 0$, $\gamma_\star = 2$ (pure second-layer problem, need
$m > 2n$, but there is no data dimension); for $\delta = 1$, $\gamma_\star = 4/5$; and
$\gamma_\star \to 0$ as $\delta \to \infty$ (high-dimensional data requires less width
relative to samples, since the first-layer parameters grow with~$d$).

The formula $\gamma_\star = 4/(2 + 3\delta)$ is confirmed by numerical solution of the
self-consistent equation for all $\delta > 0$.
\end{proof}

% --- Figure 3: Critical width ratio for isotropic data ---
\begin{figure}[t]
\centering
\begin{tikzpicture}
\begin{axis}[
  width=0.75\textwidth,
  height=0.5\textwidth,
  xlabel={Data dimension ratio $\delta = d/n$},
  ylabel={Critical width ratio $\gamma^\star$},
  title={Critical Width Ratio for Isotropic Data ($\Sigma = I$)},
  xmin=0, xmax=3,
  ymin=0, ymax=1.6,
  grid=major,
  grid style={gray!20},
  legend style={font=\small, draw=none, fill=none, at={(0.97,0.97)}, anchor=north east},
  every axis title/.style={font=\normalsize},
]
% gamma*(delta) = 4/(2+3delta)
\addplot[blue, thick, smooth, domain=0.01:3, samples=200]
  {4/(2 + 3*x)};
\addlegendentry{$\gamma^\star(\delta) = \frac{4}{2 + 3\delta}$}
% gamma = 1 line
\addplot[dashed, gray, thin, domain=0:3] {1};
\addlegendentry{$\gamma = 1$ (interpolation threshold)}
% Shaded supercritical region
\addplot[blue, fill=blue, fill opacity=0.06, draw=none, domain=0.01:3, samples=200]
  {4/(2 + 3*x)} \closedcycle;
% Annotation
\node[blue, font=\small] at (axis cs:1.8,1.1) {All minima global};
\node[font=\small] at (axis cs:2.2,0.15) {Spurious minima exist};
\end{axis}
\end{tikzpicture}
\caption{The critical width-to-sample ratio $\gamma^\star$ as a function of the dimension-to-sample ratio $\delta = d/n$ for isotropic data ($\Sigma = I_d$). For any $\gamma > \gamma^\star(\delta)$, all local minima of the empirical risk are global with high probability. Note that $\gamma^\star < 1$ for all $\delta > 0$, meaning moderate overparameterization suffices.}
\label{fig:isotropic}
\end{figure}
