...
% Fill "All minima global" (Top - Blue)
\addplot[draw=none, fill=blue, fill opacity=0.1] coordinates {(0,0) (0,2.5) (3,2.5) (3,0)};
% Fill "Spurious minima exist" (Bottom - Red/White layering)
...
% Subcritical
\addplot[red, thick, dashed] {x^0.5};
\addlegendentry{Subcritical ($\gamma < \gamma_\star$)}

% Reference
\addplot[gray, thin] {0.5*x^0.5};
\node[gray, anchor=north west] at (axis cs:0.1, 0.15) {Slope $1/2$};

\end{loglogaxis}
\end{tikzpicture}
\caption[Scaling of the spectral gap]{Scaling of the spectral gap near the transition. Both branches exhibit a square-root scaling $|\Delta| \sim |\gamma - \gamma_\star|^{1/2}$.}
...
\node[align=center] at (5,3) {Empirical results forthcoming};
\end{tikzpicture}
\caption[Empirical validation]{Empirical validation of the phase boundary. The theoretical prediction $\gamma_\star(\delta)$ aligns with the transition point observed in numerical simulations of gradient descent.}
...
\node[align=center] at (5,3) {Empirical results forthcoming};
\end{tikzpicture}
\caption[Empirical spectral density]{Empirical spectral density of the Hessian. The histogram of eigenvalues confirms the square-root behavior at the edge of the spectrum near the critical point.}
