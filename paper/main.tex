...
\citep{choromanska2015,kawaguchi2016,safran2018,venturi2019,geiger2019,sagun2017}. Choromanska et al.\
connected neural loss surfaces to spin-glass Hamiltonians (see also Ben~Arous and Gheissari~\citep{benarous2019} for related Kac--Rice analyses in high-dimensional non-convex landscapes); Kawaguchi showed that linear
...
\citep{du2019}, Allen-Zhu et al.\ \citep{allenzhu2019}, and the NTK framework
\citep{jacot2018} established global convergence guarantees, but only when the width $m$
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Sharp topological threshold.} We identify the minimal sufficient condition for a benign landscape: a critical width-to-sample ratio $\gamma_\star$ (depending on the data covariance spectrum) that marks a topological phase transition
...
\end{figure}
  ...
  Theorem~\ref{thm:phase-transition} establishes $\gamma_\star$ as a \emph{sufficient} condition for a benign landscape; crucially, as we demonstrate in Section~\ref{sec:landscape-dynamics}, it is not \emph{necessary} for successful optimization.
  Empirical training dynamics confirm that gradient descent succeeds well below $\gamma_\star$ (Figure~\ref{fig:empirical-validation}), revealing that landscape topology alone is insufficient to predict optimization difficulty.
...
minimum \citep{choromanska2015}. This qualitative picture was sharpened in two directions.
On one hand, Kawaguchi \citep{kawaguchi2016} proved that \emph{linear} networks have no
...
\citep{safran2018} showed that two-layer ReLU networks \emph{do} harbor spurious minima
when underparameterized, while Venturi et al.\ \citep{venturi2019} gave sufficient
...
A separate line established global convergence guarantees via the NTK regime, but only at polynomial width $m = \Omega(n^6)$; see Appendix~\ref{app:extended-related}.
...
the Jacobian and kernel matrices \citep{pennington2017,louart2018,ghorbani2019}. These works
...
\begin{lemma}[Sharpened decoupling via leave-one-out]\label{lem:decoupling-sharp}
  ...
  \[
    \bigl\|\nabla^2 L(\theta_c) - H_{\mathrm{dec}}(\theta_c)\bigr\|_{\op} = O_P\!\left(n^{-1/2}\right).
  \]
\end{lemma}

\begin{remark}[Rate sufficient for main results]\label{rem:rate-sufficient}
  The weaker $O_P(n^{-1/2})$ rate established here suffices for Theorems~\ref{thm:critical-ratio}, \ref{thm:phase-transition}, and~\ref{thm:scaling} (linear spectral gap scaling). The sharper $O_P(n^{-2/3})$ rate, which would yield Tracy--Widom fluctuations and the critical window $|\gamma-\gamma_\star| = O(n^{-2/3})$, requires extending edge universality to the joint block structure of $H_{\mathrm{dec}}$ across neurons sharing $X$. This extension is not currently available in the literature; we treat the Tracy--Widom scaling in Remark~\ref{rem:crossover} as conditional on this extension.
\end{remark}
...
\begin{theorem}[Sharp phase transition]\label{thm:phase-transition}
  ...
  Theorem~\ref{thm:critical-ratio}. The bounded-loss hypothesis $L(\theta_c) \le C$ is non-vacuous by Proposition~\ref{prop:coercivity}, which establishes that no critical point of a ReLU network with sub-Gaussian data can have unbounded loss:
...
\begin{lemma}[Uniform Jacobian bound]\label{lem:jacobian-uniform}
  Under Assumptions~\ref{ass:data}--\ref{ass:labels}, for any $R > 0$, $\sup_{\|\theta\| \le R} \|J(\theta)\|_{\op} = O_P(\sqrt{n})$.
\end{lemma}

\begin{proof}
  The Jacobian has entries $J_{i,(j-1)d+k} = (a_j/\sqrt{m}) \cdot \mathbf{1}[w_j^\top x_i > 0] \cdot x_{ik}$. For fixed $\theta$, $\|J(\theta)\|_{\op} \le (\|a\|_\infty/\sqrt{m}) \cdot \|X\|_{\op} \cdot \sqrt{m} = \|a\|_\infty \cdot \|X\|_{\op}$, and $\|X\|_{\op} = O_P(\sqrt{n})$ by Bai--Yin.

  For uniformity over $\|\theta\| \le R$: the Jacobian changes when activation patterns flip. Cover the parameter ball with an $\varepsilon$-net of size $\exp(O(p \log(R/\varepsilon)))$. At each net point the bound holds; between net points, the Jacobian changes only when some $w_j^\top x_i$ crosses zero. By Gaussian anti-concentration, the number of such flips across the net is controlled. Choosing $\varepsilon = 1/n$ makes the net argument close. See Vershynin's \textit{High-Dimensional Probability} Theorem~4.6.1 for the covering bound.
\end{proof}
...
\begin{remark}[Landscape geometry vs.\ optimization dynamics]\label{rem:landscape-dynamics}
  Theorem~\ref{thm:phase-transition} characterizes the \emph{static geometry} of the loss surface and does not predict optimization outcomes directly. The obvious implication -- that gradient descent should fail below $\gamma_\star$ -- is empirically false. In 960 independent optimization runs spanning $\gamma \in [0.2\gamma_\star, 2.0\gamma_\star]$, every converged run found a global minimum (Section~\ref{sec:landscape-dynamics}), including at $\gamma = 0.2\gamma_\star$, deep in the subcritical regime where the expected number of spurious minima is $e^{\Omega(n)}$. The gap between landscape topology and optimization difficulty is not a finite-size artifact: it persists at $n = 1500$ (Table~\ref{tab:hero-run}). We return to possible explanations -- vanishing basin widths, optimization inductive bias -- in Section~\ref{sec:landscape-dynamics}.
\end{remark}
...
\begin{theorem}[Spectral gap scaling law]\label{thm:scaling}
  ...
  \begin{remark}[Finite-size crossover]\label{rem:crossover}
    ...
    from Lemma~\ref{lem:decoupling-sharp} holds for the gated block structure
    (see Remarks~\ref{rem:rate-sufficient} and~\ref{rem:bai-yin-gap} for discussion), the error improves to
...
and Zeitouni~\citep{anderson2010}), adapted to our ``gated'' setting. The key additional
...
\begin{remark}[Discussion of Assumption~\ref{ass:mean-field}]\label{rem:mean-field}
  ...
  standard random-feature analysis \citep{jacot2018,mei2018}. We elevate this to an
  ...
  assumption holds rigorously for convex random feature models \citep{mei2018}, where the
...
and sign flip \citep{safran2018}: there exists a permutation $\pi$ on $[m_{\mathrm{teacher}}]$
...
From the self-consistent equation for the Stieltjes transform, the edge $\lambda_-(\gamma)$
...
solution to the self-consistent equation for the Stieltjes transform in the upper
...
\begin{theorem}[Deterministic equivalent for the gated Hessian]\label{thm:det-equiv}
  ...
  equation. Then for every $\varepsilon > 0$, uniformly
...
the anisotropic local law machinery of Knowles and Yin~\citep{knowlesyin2017}, adapted
...
follows the template of Auffinger, Ben Arous, and \v{C}ern\'{y}~\citep{auffinger2013}
...
$p$-spin model~\citep{auffinger2013}. In the spin glass setting, the covariance of the
...
the Stieltjes transform fixed-point equation. By the
universality results of Tao and Vu \citep{tao2012} and Erd\H{o}s, Yau, and
Yin \citep{erdos2012}, the bulk and edge eigenvalue statistics of sample covariance
...
\begin{proof}[Partial verification]
...
\centering
\small
\caption{Concrete lower bounds on spurious minimum count from Theorem~\ref{thm:phase-transition}(b) at $(\delta, \gamma) = (1, 0.5\gamma_\star)$, where $\gamma_\star = 4/5$. The bound $\exp(c'(\gamma_\star - \gamma)^2 n)$ is evaluated with $c' = 0.3$ (derived from the Kac--Rice analysis). Even loose bounds predict exponentially many spurious minima, yet optimization consistently avoids them.}\label{tab:spurious-bounds}
\begin{tabular}{@{}ccc@{}}
\toprule
...
We further investigate the basin structure through exhaustive search in the 2D underparameterized regime ($d=40$, $m=5$, $n=80$, random labels). Across 100 random initializations, we identify 49 distinct loss basins, providing direct empirical evidence that spurious minima exist. To quantify their accessibility, we measure the basin width: the maximum initialization distance from a spurious minimum that still converges to it. Table~\ref{tab:basin-structure} summarizes these findings. Table 4 says: we find 49 distinct basins with the global minimum at loss 0.013 and spurious minima reaching up to 2.57; critically, the maximum trap distance is only $\approx 0.1$, meaning random initialization almost always escapes to the global basin.

\begin{table}[h]
\centering
\small
\caption{Basin structure analysis in the underparameterized regime ($d=40$, $m=5$, $n=80$, random labels).}\label{tab:basin-structure}
\begin{tabular}{@{}lc@{}}
\toprule
Metric & Value \\
\midrule
...
The key insight is that while 49 distinct basins exist, they are both \emph{narrow} (trap radius $<0.1$) and \emph{stable} (noise does not help escape). This provides direct evidence for the vanishing basin width mechanism: spurious minima exist mathematically but are practically inaccessible to gradient-based optimization.
...
\begin{itemize}
  ...
  near-minimal norm solutions \citep{gunasekar2018}, and gradient flow in overparameterized
  networks converges to the max-margin classifier in parameter space \citep{lyu2020}. These
...
\paragraph{Polynomial-width convergence guarantees.}
...
polynomially in the sample size: $m = \Omega(n^2)$ \citep{du2019},
$\Omega(n^4)$ \citep{allenzhu2019}, or worse \citep{zou2020}. Mei, Montanari, and Nguyen \citep{mei2018} developed a mean-field theory of two-layer networks that captures the proportional regime but focuses on the population risk rather than the landscape topology. These analyses typically
proceed through the Neural Tangent Kernel (NTK) regime \citep{jacot2018}, where the
