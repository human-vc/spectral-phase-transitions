...
\node[blue, font=\footnotesize] at (axis cs:1.2,0.9) {All minima global};
\node[red, font=\footnotesize] at (axis cs:0.25,-0.75) {minima exist};
...
\begin{axis}[
  ...
  grid=major,
  grid style={gray!20},
  legend style={font=\footnotesize, draw=none, fill=none, at={(0.97,0.97)}, anchor=north east},
]
...
\caption{The critical width-to-sample ratio $\gamma^\star$ as a function of $\delta = d/n$ for isotropic data ($\Sigma = I_d$). For any $\gamma > \gamma^\star(\delta)$, all local minima are global w.h.p. Note $\gamma^\star < 1$ for all $\delta > 0$: moderate overparameterization suffices.}
\label{fig:isotropic}
\end{figure}
