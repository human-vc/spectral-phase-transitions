...
\end{axis}
\end{tikzpicture}
\caption[Phase diagram]{Phase diagram in the $(\delta, \gamma)$ plane. The solid curve $\gamma_\star(\delta) = \frac{4}{2+3\delta}$ marks the topological phase transition: above this boundary, Theorem~\ref{thm:phase-transition}(a) guarantees that all local minima are global; below it, exponentially many spurious critical points exist in the landscape (Theorem~\ref{thm:phase-transition}(b)).}
...
(the noise floor). Fix $C > L_\star$ and consider the compact sublevel set
$S_C = \{\theta : L(\theta) \le C\}$. The spectral gap bound ensures that every
...
\begin{enumerate}[label=(\roman*)]
  ...
  No spurious local minima were detected.
