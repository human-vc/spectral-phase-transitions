...
$n = 200$ training images, apply PCA to reduce to $d$ dimensions, and whiten the result
(so the empirical covariance is approximately $I_d$). We then sweep $\gamma = m/n$ from
$0.3$ to $2.5$ for $\delta \in \{0.15, 0.25, 0.40\}$, training two-layer ReLU networks
with gradient flow ($\eta = 5 \times 10^{-4}$, $20{,}000$ steps, median over 3 seeds).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/fig7_mnist_phase_boundary.pdf}
\caption[MNIST phase boundary]{Phase boundary on whitened MNIST data ($n = 200$).
...
with limited input features. As $\delta$ increases, the loss drops sharply above the
theoretical curve, consistent with the predicted phase transition. The transition is softer
...
the qualitative agreement provides empirical support for the universality conjecture.
