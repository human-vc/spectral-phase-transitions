...
\begin{remark}[A posteriori justification of Assumption~\ref{ass:mean-field} in part~(a)]\label{rem:mean-field-aposteriori}
  In the supercritical regime $\gamma > \gamma_\star$, part~(a) implies that every local
  minimum is global. By Proposition~\ref{prop:mean-field-global},
  Assumption~\ref{ass:mean-field} holds at all global minimizers. The theorem statement
  invokes Assumption~\ref{ass:mean-field} at arbitrary critical points, but for the
  conclusion of part~(a) it suffices to verify the assumption at local minima (the spectral
  gap argument applies to saddle points without the mean-field condition, since they are not
  local minima regardless). The dependence on Assumption~\ref{ass:mean-field} in part~(a)
...
\begin{corollary}[Simplified form]\label{cor:simplified}
  ...
  \]
  where the last step uses $\frac{4}{2+3\delta} = \frac{2}{1 + \frac{3}{2}\delta} = 2(1 - \frac{3}{2}\delta + O(\delta^2))$ and matching the linear coefficient confirms agreement through first order in $\delta$.
