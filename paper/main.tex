...
Strikingly, gradient-based optimizers navigate the subcritical ($\gamma < \gamma_\star$) regime
...
Two distinct questions underlie this puzzle: \emph{what is the geometry of the loss
surface itself}, and \emph{why do optimizers navigate it so effectively}? This paper
gives a sharp answer to the first question and connects it to the second through
large-scale experiments.

Prior work has approached the landscape question from several angles
\cite{choromanska2015,kawaguchi2016,safran2018,venturi2019}. Choromanska et al.\
connected neural loss surfaces to spin-glass Hamiltonians; Kawaguchi showed that linear
...
\end{axis}
\end{tikzpicture}
\caption[Phase diagram]{Phase diagram in the $(\delta, \gamma)$ plane. The solid curve shows the exact critical ratio $\gamma_\star(\delta) = 2(1-2\delta)/(1-\delta-\delta^2)$ for $\delta < 1/2$; the dashed curve shows the first-order approximation $4/(2+3\delta)$, which extends to all $\delta > 0$ and serves as the continuation for $\delta \ge 1/2$. The shading uses the approximate formula. Above $\gamma_\star$, Theorem~\ref{thm:phase-transition}(a) guarantees that all local minima are global; below it, exponentially many spurious critical points exist (Theorem~\ref{thm:phase-transition}(b)).}
...
The spin-glass analogy of Choromanska et al.\ \cite{choromanska2015} and Kawaguchi's
\cite{kawaguchi2016} proof that linear networks have no spurious minima launched a line of
work on loss surface topology. Safran and Shamir \cite{safran2018} constructed explicit
spurious minima for underparameterized ReLU networks; Venturi et al.\ \cite{venturi2019}
gave conditions for their absence. Our contribution is to locate the exact transition
point between these regimes as a function of the width-to-sample ratio.

\paragraph{Overparameterization thresholds.}
The convergence guarantees of Du et al.\ \cite{du2019}, Allen-Zhu et al.\
\cite{allenzhu2019}, and Zou et al.\ \cite{zou2020} require $m = \mathrm{poly}(n)$
and operate through NTK linearization \cite{jacot2018}. Our results are qualitatively
different: the transition occurs at $m = \Theta(n)$, determined by the spectral
interaction between the data covariance and the gated Hessian blocks.

\paragraph{Random matrix theory in learning.}
Pennington and Worah \cite{pennington2017} and Louart et al.\ \cite{louart2018}
brought Marchenko--Pastur and free probability tools to the analysis of neural Jacobians
and kernel matrices. Our spectral decoupling applies these tools directly to the Hessian
of the empirical risk at critical points, rather than to the network's feature map.
...
\begin{remark}\label{rem:delta1}
  ...
  the gated sample covariance has aspect ratio $2\delta = 2 > 1$ and requires a
  regularized continuation of the Marchenko--Pastur analysis; the approximation
  $4/(2+3\delta)$ remains well-behaved and provides a useful guideline.)
  This means that $m \ge \lceil 4n/5 \rceil$ hidden neurons approximately suffice
  to eliminate all spurious local minima---a dramatic improvement over prior
...
\begin{remark}[Mean-field independence]\label{rem:mean-field}
  The spectral decoupling treats the activation patterns $S_j = \{i : w_j^\top x_i > 0\}$
  for different neurons $j$ as approximately independent. In reality, these patterns
  share the data matrix~$X$ and are therefore correlated. However, for
  wide networks in the proportional limit, these correlations decay as $O(1/\sqrt{m})$:
  the overlap $|S_j \cap S_k|/n$ concentrates around $1/4 + (1/2\pi)\arcsin(\langle
  \hat{w}_j, \hat{w}_k\rangle)$ by standard Gaussian comparison inequalities, and for
  independently drawn weights, $\langle \hat{w}_j, \hat{w}_k\rangle = O(d^{-1/2})$.
  This mean-field approximation is standard in the analysis of random feature models
  and neural tangent kernels \cite{jacot2018}, and is justified rigorously in the
  proportional limit by the universality results of Section~\ref{sec:universality}.
\end{remark}
...
change the homotopy type---a contradiction with $\theta_c$ being a strict local minimum
...
\]
Thus the \emph{exact} critical ratio for $\delta < 1/2$ is:
...
\begin{corollary}[Simplified form]\label{cor:simplified}
  ...
  More precisely, the relative error between the exact and simplified formulas is:
  \[
    \frac{\gamma_\star^{\mathrm{exact}} - \gamma_\star^{\mathrm{simple}}}
         {\gamma_\star^{\mathrm{exact}}}
    = \frac{\delta^2(2+3\delta) - 2\delta^2(1-2\delta)}
           {(1-\delta-\delta^2)(2+3\delta)}
    = O(\delta^2).
  \]
  At $\delta = 1/4$, both formulas give $\gamma_\star = 16/11 \approx 1.455$.
...
$\gamma_\star$. Our experiments reveal a more nuanced picture.
...
\begin{enumerate}[label=(\roman*)]
  ...
  $G(\theta) = \|\nabla L(\theta)\|^2$ using Adam followed by L-BFGS refinement, which
  converges to the \emph{nearest} critical point regardless of its type. Across $960$
  independent runs ($4$ values of $\delta$, $12$ values of $\gamma/\gamma_\star$ from $0.2$
  to $2.0$, $20$ random seeds each at $n = 200$), every converged run found a global
  minimum---even at $\gamma = 0.2\,\gamma_\star$, deep in the subcritical regime.
...
\begin{itemize}
  ...
  lower than the asymptotic theory predicts. However, a high-resolution verification
  at $n = 1500$ with $\delta = 0.3$ (Gaussian teacher-student data, trained with Adam,
  20 seeds per $\gamma$, 10 values of $\gamma/\gamma_\star$ from $0.3$ to $2.0$)
  achieved $100\%$ global convergence at every tested point---including at
  $\gamma = 0.3\,\gamma_\star$, deep in the subcritical regime. While the primary
  GNM experiments (Section~\ref{sec:landscape-dynamics}) used gradient-based critical
  point search to decouple landscape geometry from optimizer bias, this Adam-based
  verification confirms that practical optimizers navigate the subcritical regime
  even more effectively, and that the landscape--dynamics gap does not close at
  moderate~$n$.
...
this phenomenon: the landscape is \emph{provably} rugged below $\gamma_\star$, yet
optimization is \emph{empirically} smooth. This places $\gamma_\star$ as a
...
Perhaps the most striking finding is the disconnect between landscape geometry and
...
of optimization difficulty. This reinforces an emerging picture: the success
of neural network training is driven not by the absence of bad critical points, but by
the inductive biases of the optimizer that steer trajectories away from them.
